...
- Clipboard Sync is a native macOS menu-bar, Windows system-tray, and Linux Qt 6 application. Linux supports encrypted text/image sync, explicit clipboard-file transfer, and port forwarding; input sharing is capability-detected but is not implemented and must not be presented as available.
